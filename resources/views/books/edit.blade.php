<x-app-layout>
    <x-slot name="header">
        ...
        <div class="lib-header-wrap">
            <div>
                <h2 class="lib-header-title">{{ __('Edit Volume') }}</h2>
                <p class="lib-header-subtitle">{{ __('Revise catalogue details for this work') }}</p>
            </div>
        </div>
    </x-slot>
    ...
    <div class="lib-page py-10">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="lib-form-shell">
                ...
                <form method="POST" action="{{ route('books.update', $book->id) }}" class="lib-form-card" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                    <div class="lib-form-head">
                        <p class="lib-form-head-meta">{{ __('Catalogued volume') }}</p>
                        <h3 class="lib-form-head-title">{{ __('Edit Volume') }}</h3>
                    </div>

                    <div class="lib-form-body">
                        <div>
                            <label for="title" class="lib-field-label">{{ __('Book Title') }}</label>
                            <input id="title" name="title" type="text" class="lib-input" value="{{ old('title', $book->title) }}"  autofocus>
                            @error('title') <p class="lib-input-error">{{ $message }}</p> @enderror
                        </div>

                        <div>
                            <label for="author" class="lib-field-label">{{ __('Author') }}</label>
                            <input id="author" name="author" type="text" class="lib-input" value="{{ old('author', $book->author) }}" >
                        ...
                        <div>
                            ...
                            <input id="isbn" name="isbn" type="text" class="lib-input" value="{{ old('isbn', $book->isbn ?? '') }}"
                            pattern="[0-9\-]+" inputmode="numeric" placeholder="xxx-xxxxxxxxxx" oninput="this.value = this.value.replace(/[^0-9-]g,'')">
                        ...
                        <div>
                            ...
                            <input id="published_date" name="published_date" type="date" class="lib-input" max="{{ date('m-d-Y') }} "value="{{ old('published_date', $book->published_date) }}">
                        ...
                        <div>
                            ...
                            <textarea id="description" name="description" class="lib-textarea">{{ old('description', $book->description) }}</textarea>
