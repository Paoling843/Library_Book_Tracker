<x-app-layout>
    <x-slot name="header">
        ...
        <div>
            <h2 class="lib-header-title">{{ __('Add Category') }}</h2>
            <p class="lib-header-subtitle">{{ __('Register a new classification for the stacks') }}</p>
        </div>
    </x-slot>
    ...
    <div class="lib-page py-10">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="lib-form-shell">
                <form method="POST" action="{{ route('categories.store') }}" class="lib-form-card">
                    @csrf
                    <div class="lib-form-head">
                        <p>{{ __('New classification') }}</p>
                        <h3>{{ __('Add Category') }}</h3>
                    </div>
                    <div class="lib-form-body">
                        <label for="name" class="lib-field-label">{{ __('Category Name') }}</label>
                        <input id="name" type="text" name="name" value="{{ old('name') }}" class="lib-input" required autofocus>
                    ...
                    <div class="lib-form-footer">
                        ...
                        <button type="submit" class="lib-btn lib-btn-primary">{{ __('Save Category') }}</button>
                    </div>
                </form>
            </div>
        </div>
